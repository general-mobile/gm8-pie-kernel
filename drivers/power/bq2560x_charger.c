								...
								int disable)
{
	...
	pr_err("reason=%d requested_disable=%d disabled_status=%d charge_enabled=%d\n",
					reason, disable, disabled,bq->charge_enabled);
...
void static runin_work(struct bq2560x *bq, int batt_capacity)
{
	...
	if (batt_capacity >= 80) {
		...
		pr_debug("bq2560x_get_prop_batt_capacity < 50\n");
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		if(val->intval) 
		{
			...
			power_supply_set_online(bq->usb_psy, true);
...
static int bq2560x_update_charging_profile(struct bq2560x *bq)
{
	...
	if (bq->jeita_active) {
		...
		if (prop.intval == POWER_SUPPLY_TYPE_USB_DCP || prop.intval == POWER_SUPPLY_TYPE_USB_CDP) {
			...
			bq->usb_psy_ma = bq->platform_data->usb.ichg;
		}
	}
	
	icl = bq->usb_psy_ma;
	if (bq->usb_psy_ma < chg_ma && !bq->jeita_active) {
	...
	if (ret < 0) {
		pr_err("couldn't read USB voltage_now property, ret=%d\n", ret);
		return ret;
	}

	if( prop.intval < (3400 * 1000))
	{
		chg_ma = 300;
		Trickle_charge = 1;
		pr_err("is Trickle_charge in kernel\n");
	} else {
		Trickle_charge = 0;
		pr_err("is not Trickle_charge in kernel\n");
	}
...
static void bq2560x_external_power_changed(struct power_supply *psy)
{
	...
	ret = bq2560x_get_hiz_mode(bq, &isHIZmode);
	if(isHIZmode) return;
	...
	if (bq->usb_present && (current_limit != 2)){ //for pc sleep
		if (prop.intval == 0){
...
static irqreturn_t bq2560x_charger_interrupt(int irq, void *dev_id)
{
	...
	ret = bq2560x_get_hiz_mode(bq, &isHIZmode);
	pr_info("++ %s bq->power_good =%d isHIZmode = %d .\n",__func__,bq->power_good,isHIZmode);
	
	if((!isHIZmode) &&!bq->power_good) {
	    if(bq->usb_present) {
	...
	power_supply_changed(&bq->batt_psy);
	usb_suspend_flag = bq->usb_present;
	return IRQ_HANDLED;
}
					...
					const struct i2c_device_id *id)
{
	...
	//i2c_devinfo_device_write("CHG:1;");    //for runin test
