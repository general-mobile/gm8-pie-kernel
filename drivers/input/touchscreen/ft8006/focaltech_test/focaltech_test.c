...
static int fts_test_entry(char *ini_file_name)
{
    ...
    {
	    fts_test_save_test_data("testdata_fail.csv", test_data.store_all_data, data_len);
	    fts_test_save_test_data("testresult_fail.txt", test_data.testresult, test_data.testresult_len);	
    }
...
static ssize_t fts_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    fts_esdcheck_set_status(DISABLE);
    fts_esdcheck_switch(DISABLE);
#endif
   
   while(isESDQueue_flag == 1) {
   	mdelay(1000);
	if(isESDQueue_flag == 1){
		isESDQueue_flag = 0;
		printk("[FTS][TEST]TP self test time out\n");	
		break;
	}
	printk("[FTS][TEST]TP self test is working,please wait it done\n");	
   }
   
    fts_test_entry(fwname);

#if defined(FTS_ESDCHECK_EN) && (FTS_ESDCHECK_EN)
    fts_esdcheck_set_status(ENABLE);
