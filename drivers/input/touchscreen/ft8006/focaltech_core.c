...
//2017.9.14 chenbaicheng add for tp info start
#ifdef SUPPORT_READ_TP_VERSION	
char dev_info[100];
bool initialized = false;
void lct_tp_info(struct i2c_client *client,char *module)
{
    char info[50] = {0};
	u8 tp_ic_fm_ver,lcd_ic_ver;
	int ret = 0,fw_ver;
    ...
    ret = fts_i2c_read_reg(client, FTS_REG_FW_VER, &tp_ic_fm_ver);
	if (ret < 0) {
		FTS_ERROR("get fw ver fail");
		FTS_FUNC_EXIT();
		return;
	}

    ret = fts_i2c_read_reg(client, FTS_REG_LIC_VER, &lcd_ic_ver);
	if (ret < 0) {
		FTS_ERROR("get fw ver fail");
		FTS_FUNC_EXIT();
		return;
	...
	fw_ver = (tp_ic_fm_ver << 8) |lcd_ic_ver ;

	snprintf(info, sizeof(info), "[fw]0x%x,[ic]FT8006M", fw_ver); 
	snprintf(dev_info, sizeof(dev_info), "[Vendor]%s,[fw]0x%x,[ic]FT8006M", module, fw_ver);
	if(!module)
...
int focaltech_gesture_enable = 1;
...
#if FTS_GESTURE_EN
    fts_gesture_resume(ts_data->client);
#endif
...
static int __init fts_ts_init(void)
{
	...
	printk("ID0_status = %d, ID1_status = %d ,This is not Focal IC,exit init.\n",ID0_status,ID1_status);
        return -1;
    }		
