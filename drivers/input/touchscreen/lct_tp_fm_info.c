...
extern int himax_probe_success;
static ssize_t tp_proc_tp_info_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	if(1 == himax_probe_success)
	{
		//E("%s: config_ver = 0x%2.2x--------------\n", __func__, ic_data->vendor_config_ver);
		memset(tp_ver_show_str, 0, sizeof(tp_ver_show_str));
		sprintf(tp_ver_show_str, "[FW]0x%x,[IC]HX83102b", ic_data->vendor_config_ver);
	}
