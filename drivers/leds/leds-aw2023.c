...
static void aw2023_brightness_work(struct work_struct *work)
{
	...
	if (led->cdev.brightness > 0) {
		...
		if(led->id == 3){
		   aw2023_write(led, AW2023_REG_LCFG0 + 1,
					(AW2023_LED_MANUAL_MODE_MASK | led->pdata->led_current));
		   aw2023_write(led, AW2023_REG_LCFG0 + 2,
					(AW2023_LED_MANUAL_MODE_MASK | led->pdata->led_current));
		   aw2023_write(led, AW2023_REG_PWM0 + 1, 52);
		   aw2023_write(led, AW2023_REG_PWM0 + 2, 79);
		   aw2023_read(led, AW2023_REG_LEDEN, &val);
		   aw2023_write(led, AW2023_REG_LEDEN, val | 6);   // led->id << 1
		}else{
				...
				const struct i2c_device_id *id)
{
	...
	pr_err("tom han enjoy probe1\n");
	if (node == NULL)
//		return -EINVAL;
	{
		pr_err("tom han node ==null\n");		
		return -EINVAL;	
	}
...
//		return -EINVAL;
	...
	if(ret) {
		pr_err("tom han aw2023_power_on fail \n");
		return -EINVAL ;
	}
...
//		dev_err(&client->dev, "Check chip id error\n");
		pr_err("tom han Check chip id error \n");
		goto free_led_arry;
	}
...
//		dev_err(&client->dev, "parsed node error\n");
		pr_err("parsed node error\n");
		goto free_led_arry;
	}

	i2c_set_clientdata(client, led_array);
	pr_err("hom han aw2023_probe_ok\n");
	return 0;
