...
static unsigned int p_tas2560_irq_config[] = {
	TAS2560_INT_GEN_REG,	0x01,	0xff,/* enable spk OC and OV*/
...
static int tas2560_i2c_load_data(struct tas2560_priv *pTAS2560, unsigned int *pData)
{
	...
	do {
		...
		if (nRegister == TAS2560_MSLEEP) {
			msleep(nData[0]);
			dev_err(pTAS2560->dev, "%s, msleep = %d\n",
				__func__, nData[0]);
		} else if (nRegister == TAS2560_MDELAY) {
			mdelay(nData[0]);
			dev_err(pTAS2560->dev, "%s, mdelay = %d\n",
				__func__, nData[0]);
		} else {
		...
		unsigned int freq)
{
	...
	dev_err(pTAS2560->dev, "%s, clkid=%d, freq=%d\n", __func__, clk_id, freq);
...
int tas2560_setupPLL(struct tas2560_priv *pTAS2560, int pll_clkin)
{
	...
	if (power & TAS2560_PWR_BIT_MASK) {
		dev_err(pTAS2560->dev, "power down to update PLL\n");
...
int tas2560_parse_dt(struct device *dev, struct tas2560_priv *pTAS2560)
{
	...
	if (rc) {
		...
		dev_err(pTAS2560->dev, "ti,load=%d", pTAS2560->mnLoad);
	}
	...
	if (rc) {
		...
		dev_err(pTAS2560->dev, "ti,asi-format=%d", pTAS2560->mnASIFormat);
	}
	...
	if (!gpio_is_valid(pTAS2560->mnResetGPIO)) {
		...
		dev_err(pTAS2560->dev, "ti,reset-gpio=%d", pTAS2560->mnResetGPIO);
	}
	...
	if (!gpio_is_valid(pTAS2560->mnIRQGPIO)) {
		...
		dev_err(pTAS2560->dev, "ti,irq-gpio=%d", pTAS2560->mnIRQGPIO);
	}
	...
	if (rc) {
		...
		dev_err(pTAS2560->dev, "ti,ppg=%d", pTAS2560->mnPPG);
	}
	...
	if (rc) {
		...
		dev_err(pTAS2560->dev, "ti,pll=%d", pTAS2560->mnPLL);
	}
...
int tas2560_LoadConfig(struct tas2560_priv *pTAS2560, bool bPowerOn)
{
	...
	if (bPowerOn) {
		dev_err(pTAS2560->dev, "%s power down to load config\n", __func__);
	...
	if (bPowerOn) {
		dev_err(pTAS2560->dev, "%s power up\n", __func__);
...
int tas2560_enable(struct tas2560_priv *pTAS2560, bool bEnable)
{
	...
	if (bEnable) {
		...
		if (pTAS2560->mbPowerUp) {
			dev_err(pTAS2560->dev, "%s power down\n", __func__);
			if (hrtimer_active(&pTAS2560->mtimer))
				hrtimer_cancel(&pTAS2560->mtimer);
			pTAS2560->enableIRQ(pTAS2560, false);
			nResult = tas2560_i2c_load_data(pTAS2560, p_tas2560_shutdown_data);
