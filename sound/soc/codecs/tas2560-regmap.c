	...
	unsigned int reg, unsigned int *pValue)
{
	...
	if (nResult < 0)
		...
		dev_err(pTAS2560->dev, "%s: BOOK:PAGE:REG %u:%u:%u\n", __func__,
			TAS2560_BOOK_ID(reg), TAS2560_PAGE_ID(reg),
	...
	unsigned int reg, unsigned int value)
{
	...
	if (nResult < 0)
		...
		dev_err(pTAS2560->dev, "%s: BOOK:PAGE:REG %u:%u:%u, VAL: 0x%02x\n",
	...
	unsigned int reg, unsigned char *pData, unsigned int nLength)
{
	...
	if (nResult < 0)
		...
		dev_err(pTAS2560->dev, "%s: BOOK:PAGE:REG %u:%u:%u, len: 0x%02x\n",
			__func__, TAS2560_BOOK_ID(reg), TAS2560_PAGE_ID(reg),
			 ...
			 unsigned int mask, unsigned int value)
{
	...
	if (nResult < 0)
		...
		dev_err(pTAS2560->dev, "%s: BOOK:PAGE:REG %u:%u:%u, mask: 0x%x, val=0x%x\n",
			__func__, TAS2560_BOOK_ID(reg), TAS2560_PAGE_ID(reg),
...
static void irq_work_routine(struct work_struct *work)
{
	...
	if (((nDevInt1Status & 0xfc) != 0) || ((nDevInt2Status & 0xc0) != 0)) {
		...
		dev_err(pTAS2560->dev, "IRQ status : 0x%x, 0x%x\n",
				nDevInt1Status, nDevInt2Status);
		nCounter = 2;
		...
		while (nCounter > 0) {
			...
			if (nCounter > 0) {
				...
				dev_err(pTAS2560->dev, "PowSts B: 0x%x, check again after 10ms\n",
					nDevInt1Status);
				msleep(10);
			}
		}
...
reload:
	...
	if (!hrtimer_active(&pTAS2560->mtimer)) {
		dev_err(pTAS2560->dev, "%s, start timer\n", __func__);
...
static int tas2560_runtime_suspend(struct tas2560_priv *pTAS2560)
{
	...
	if (hrtimer_active(&pTAS2560->mtimer)) {
		dev_err(pTAS2560->dev, "cancel die temp timer\n");
		hrtimer_cancel(&pTAS2560->mtimer);
	}

	if (delayed_work_pending(&pTAS2560->irq_work)) {
		dev_err(pTAS2560->dev, "cancel IRQ work\n");
...
static int tas2560_runtime_resume(struct tas2560_priv *pTAS2560)
{
	...
	if (pTAS2560->mbPowerUp) {
		if (!hrtimer_active(&pTAS2560->mtimer)) {
			dev_err(pTAS2560->dev, "%s, start check timer\n", __func__);
			hrtimer_start(&pTAS2560->mtimer,
				ns_to_ktime((u64)CHECK_PERIOD * NSEC_PER_MSEC), HRTIMER_MODE_REL);
		}
	}
...
#endif
	pr_err ("%s SMARTPA:1 \n", __func__);
        i2c_devinfo_device_write("SMARTPA:1;");
...
{		mutex_destroy(&pTAS2560->dev_lock);
	pr_err ("%s SMARTPA:0 \n", __func__);
        i2c_devinfo_device_write("SMARTPA:0;");
}
