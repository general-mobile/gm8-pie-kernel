			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		dev_err(pTAS2560->dev, "SND_SOC_DAPM_POST_PMU");
	break;
	case SND_SOC_DAPM_POST_PMD:
		dev_err(pTAS2560->dev, "SND_SOC_DAPM_POST_PMD");
	break;
	}
...
static int tas2560_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	dev_err(pTAS2560->dev, "%s, format=0x%x\n", __func__, fmt);
			...
			struct snd_ctl_elem_value *pUcontrol)
{
	...
	dev_err(pCodec->dev, "%s:load = 0x%x\n", __func__,
			pTAS2560->mnLoad);
				...
				struct snd_ctl_elem_value *pValue)
{
	...
	dev_err(codec->dev, "tas2560_power_ctrl_get = 0x%x\n",
					pTAS2560->mbPowerUp);
