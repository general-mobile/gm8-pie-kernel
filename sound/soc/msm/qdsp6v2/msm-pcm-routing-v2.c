...
/*lc mike_zhu  20170929 for audio*/
...
#ifdef SMART_AMP
extern int32_t adm_smartamp_profile_set (int32_t port_id, u8 *profile_number
        , uint8_t sz);
extern int32_t adm_smartamp_set_re(int32_t port_id, u8 *re, uint8_t sz);

static int msm_routing_set_profile(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	int32_t ret = 0;
	uint32_t profile = ucontrol->value.integer.value[0];
	mutex_lock(&routing_lock);
	ret = adm_smartamp_profile_set(RX_PORT_ID, (u8*)&profile, 4);
	mutex_unlock(&routing_lock);
	return 0;
}


static int smartamp_get_re (struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int smartamp_set_re (struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	int32_t ret = 0;
	//struct soc_mixer_control *mc =
	//	(struct soc_mixer_control *)kcontrol->private_value;
	uint32_t re = ucontrol->value.integer.value[0];

	printk(KERN_INFO "[smartamp] setting re=0x%x", re);
	mutex_lock(&routing_lock);
	ret = adm_smartamp_set_re (TX_PORT_ID, (uint8_t*)&re, sizeof(int32_t));
	mutex_unlock(&routing_lock);
	return 0;
}

static const char *smartamp_profile_text[] = {
	"PROFILE0",
	"PROFILE1",
	"PROFILE2",
};
...
static const struct soc_enum smartamp_profile_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(smartamp_profile_text), smartamp_profile_text),
};

static const struct snd_kcontrol_new smartamp_mixer_controls[] = {
	SOC_ENUM_EXT("SMARTAMP_PROFILE", smartamp_profile_enum[0],
			msm_routing_set_profile, msm_routing_set_profile),
	SOC_SINGLE_EXT("SMARTAMP_SET_RE", 0, 0, 0x7fffffff, 0,
			smartamp_get_re, smartamp_set_re),
};
...
#ifdef SMART_AMP
	snd_soc_add_platform_controls(platform, smartamp_mixer_controls,
		ARRAY_SIZE(smartamp_mixer_controls));
#endif
	return 0;
}
